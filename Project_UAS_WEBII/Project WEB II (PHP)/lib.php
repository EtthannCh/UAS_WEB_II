...
function filterTransaksiByUser($status, $id_user)
{
    global $conn;
    $query = $conn->prepare("SELECT * FROM tbl_transaksi WHERE status_transaksi = :status_transaksi AND id_user = :id");
...
function insertToTransaksi($status, $kode_transaksi, $tanggal)
{
    global $conn;
    $query = $conn->prepare("INSERT INTO tbl_transaksi (id_user, kode_transaksi ,nama_produk, jumlah_produk, harga, pengiriman, status_transaksi, tanggal) 
    SELECT id_user, :kode_transaksi ,GROUP_CONCAT(nama_produk SEPARATOR ';'), qty_produk, SUM(total_harga), pengiriman, :status_pengiriman , :tanggal FROM tbl_cart GROUP BY id_user");
...
function findTabelKontak()
{
    global $conn;
    $query = $conn->prepare("SELECT * FROM tbl_contact_us");
    $query->execute();
    return $query->fetch();
}
...
function deleteUserCart($id_user){
    global $conn;
    $query = $conn->prepare("DELETE FROM tbl_cart WHERE id_user = :id_user");
